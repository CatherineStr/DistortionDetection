...
Public Class ImageHandler
    ...
    Dim settingsStorageRoot As SettingsStorageRoot

        Public Sub New()
        settingsStorageRoot = New SettingsStorageRoot
        settingsStorageRoot.DefaultWriter = New IniFileSettingsWriter("parameters.ini")
        settingsStorageRoot.CreateIntegerSetting("maxGapY", 5, "Максимальный разрыв на линии", "Максимальный разрыв на ветрикальной линии в прикселях, при котором эта линия ещё может считаться единой.")
        settingsStorageRoot.CreateIntegerSetting("lineMinLen", 108, "Минимальная длина линии", "Минимально возможная длина вертикальной линии.")
        settingsStorageRoot.CreateIntegerSetting("pixAreaWidth", 2, "Ширина области вокруг пикселя", "Ширина области вокруг пикселя, для которой производятся расчеты при выделении границ.")
        End Sub
    ...
    Public Sub showSettings()
        settingsStorageRoot.ShowSettingsForm(Nothing)
    End Sub
        ...
        Else
            If CInt(settingsStorageRoot.FindSetting("pixAreaWidth").ValueAsString()) < 0 Then
                logger.AddError("Некорректное значение параметра lineMinLen. Оно должно быть неотрицательным. Вычисления прерваны.")
                Return
            End If
            edgeGrayMatrix = detectEdges(grayMatrix, CInt(settingsStorageRoot.FindSetting("pixAreaWidth").ValueAsString()))
        End If
            _resImg = edgeGrayMatrix.ToRGBMatrix.ToBitmap

        logger.AddMessage("Поиск вертикальных линий")
        If CInt(settingsStorageRoot.FindSetting("lineMinLen").ValueAsString()) <= 0 Then
            logger.AddError("Некорректное значение параметра lineMinLen. Оно должно быть больше нуля. Вычисления прерваны.")
            Return
        End If
        If CInt(settingsStorageRoot.FindSetting("maxGapY").ValueAsString()) < 0 Then
            logger.AddError("Некорректное значение параметра lineMinLen. Оно должно быть неотрицательным. Вычисления прерваны.")
            Return
        End If
        Dim vertLinesAr = searchVertLines(edgeGrayMatrix, CInt(settingsStorageRoot.FindSetting("lineMinLen").ValueAsString()), CInt(settingsStorageRoot.FindSetting("maxGapY").ValueAsString()))
